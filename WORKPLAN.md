...
## Active implementation (P5c-2 COMPLETE + P5b-1-S1 MERGED + P5b-1 PHASE 2 PLAN MERGED + S2 IMPLEMENTATION NEXT)

**P5c-2 TUI automation is COMPLETE (PRs #109-#114). P5b-1-S1 cmux-terminal backend is MERGED (PR #115). P5b-1 phase 2 PLAN-DOC is MERGED (PR #119, v2.5, 6 codex review rounds). P5b-1-S2 dispatch implementation is next.**

- Current episode ID: `20260702-060728-p5b-1-phase-2-plan-doc-merged-pr-119-nex-233c`
- Tags include: `canonical-workplan`, `workplan`, `p5c-2-complete`, `p5b-1-s1-merged`, `p5b-1-phase-2-plan-merged`, `p5b-1-s2-impl-next`, `p5b-1-cmux-terminal`, `cmux-0.64.17-real-tested`, `preference-field`, `select-bg-terminal-result`
- Summary: **P5b-1 phase 2 plan-doc MERGED (PR #119, 79e3a60, 1 file +960 lines, codex R1-R6 READY-TO-MERGE).** Plan covers S2 dispatch + S2.5 CLI spike + S3 real-cmux e2e + S4 tools + S5 docs. Key design decisions: (1) `preference?: number` optional field on `TermBgBackend` with `CMUX_BACKEND_PREFERENCE=10` constant; (2) `SelectBgTerminalResult` discriminated union for none-registered vs all-unavailable distinction; (3) transparent fallback narrowed in REQ-R1(d) with cross-backend aggregation deferred to P5b-2; (4) S2.5 manual CLI spike (REQ-T1a) is a hard gate before S4. **Next: S2 implementation per Appendix B steps 2.1-2.18** (~120 LOC code + ~200 LOC tests across 9 files; 18-step executor-ready spec with verbatim ANCHORs, full test bodies, capture-and-compare verifies, paired negative controls).
