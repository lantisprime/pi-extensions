...
> 2. **Step 2.18 test code fully rewritten** per codex's R3 verification + Option A confirmation. The 4 net-new tests are now NAMED FUNCTIONS (not raw blocks) that reuse the file's existing `test(name, fn)` + `stubCtx()` helpers. No more duplicate imports (the file already imports `__resetBgTerminalBackend` and `registerBgTerminalBackend` at lines 27–30). The broken `ctx` literal shape is fixed (ui is a property of ctx, not the other way around). R3 blockers #2, #3, #4.
...
|---|---|---|
| 1 | No executor-ready step or test bodies for `test-bg-commands.mjs` (REQ-D10/D11 being MUST) | New step 2.18 with full 4-test bodies (R3 found the v2.2 bodies had 4 broken bugs — fixed in v2.3) |
...
| 2 | Step 2.18 test code is syntactically invalid (ctx literal missing closing brace; ui properties mixed with ctx properties) | Step 2.18 fully rewritten to use the file's existing `stubCtx()` helper for the ctx shape; named test functions added before `main()` |
...
| 4 | Step 2.18 placement is ambiguous (test-bg-commands.mjs uses `main()` wrapper, not top-level await) | Step 2.18 now inserts 4 named test functions before `main()` + 4 `await test("...", fn)` calls inside `main()` (matching the file's existing pattern) |
...
| 2.18 | `agents/test-fixtures/test-bg-commands.mjs` | **APPEND** 4 named test functions + 4 main() calls (R2 blocker #1, v2.3 rewrite). The existing test-bg-commands.mjs already imports `__resetBgTerminalBackend` and `registerBgTerminalBackend` at lines 27–30 — do NOT re-import them. The existing file uses a `test(name, fn)` helper (line 32) and a `stubCtx()` helper (line 248); reuse both. Insert the 4 new functions AFTER the existing `testResetCleansState` (just before `async function main()`). Full verbatim test function bodies (in order): `\n// === P5b-1-S2 REQ-D10/D11 dispatch tests (v2.3) ===\n\nasync function testBgCommandFallsThroughToTmux() {\n\t__resetBgTerminalBackend();\n\tconst cmuxDown = { name: "cmux", preference: 10, isAvailable: async () => false, launch: async () => ({ status: "failed" }), kill: async () => ({ status: "ok" }), isAlive: async () => true, list: async () => [] };\n\tconst tmuxUp = { name: "tmux", preference: 0, isAvailable: async () => true, launch: async () => ({ status: "ok", windowId: "w1" }), kill: async () => ({ status: "ok" }), isAlive: async () => true, list: async () => [] };\n\tregisterBgTerminalBackend(cmuxDown);\n\tregisterBgTerminalBackend(tmuxUp);\n\n\tconst { selectBgTerminalBackend } = await import("../lib/bg-terminal.ts");\n\tconst result = await selectBgTerminalBackend();\n\tassert.equal(result.ok, true, "fall-through must produce a usable backend");\n\tif (result.ok) assert.equal(result.backend.name, "tmux", "fall-through must select tmux when cmux is down");\n\t__resetBgTerminalBackend();\n}\n\nasync function testBgCommandReportsNoneAvailable() {\n\t__resetBgTerminalBackend();\n\tconst { selectBgTerminalBackend } = await import("../lib/bg-terminal.ts");\n\tconst result = await selectBgTerminalBackend();\n\tassert.equal(result.ok, false, "empty registry must return ok=false");\n\tif (!result.ok) assert.equal(result.reason, "none-registered", "reason must be the no-registered variant");\n\t__resetBgTerminalBackend();\n}\n\nasync function testBgCommandListsProbedBackendsWhenAllUnavailable() {\n\t__resetBgTerminalBackend();\n\tconst cmuxDown = { name: "cmux", preference: 10, isAvailable: async () => false, launch: async () => ({ status: "failed" }), kill: async () => ({ status: "ok" }), isAlive: async () => true, list: async () => [] };\n\tconst tmuxDown = { name: "tmux", preference: 0, isAvailable: async () => false, launch: async () => ({ status: "failed" }), kill: async () => ({ status: "ok" }), isAlive: async () => true, list: async () => [] };\n\tregisterBgTerminalBackend(cmuxDown);\n\tregisterBgTerminalBackend(tmuxDown);\n\n\tconst { selectBgTerminalBackend } = await import("../lib/bg-terminal.ts");\n\tconst result = await selectBgTerminalBackend();\n\tassert.equal(result.ok, false);\n\tif (!result.ok) {\n\t\tassert.equal(result.reason, "all-unavailable", "reason must be the all-unavailable variant");\n\t\tassert.equal(result.probed.length, 2, "both backends must appear in probed array");\n\t\tassert.ok(result.probed.some((p) => p.name === "cmux"), "probed must include cmux");\n\t\tassert.ok(result.probed.some((p) => p.name === "tmux"), "probed must include tmux");\n\t}\n\t__resetBgTerminalBackend();\n}\n\nasync function testBgBeforeSessionStart() {\n\t__resetBgTerminalBackend();\n\tconst { selectBgTerminalBackend } = await import("../lib/bg-terminal.ts");\n\tconst result = await selectBgTerminalBackend();\n\tassert.equal(result.ok, false);\n\tif (!result.ok) assert.equal(result.reason, "none-registered", "pre-session_start state must produce the no-registered reason");\n\t__resetBgTerminalBackend();\n}\n`. **v2.3 design note:** these 4 tests exercise `selectBgTerminalBackend()` directly, NOT `handleBgCommand()`. `handleBgCommand()` requires a full preflight + agent-resolution chain (it has to resolve "scout" via diagnostics, write a manifest to disk, etc.) which is S2 first-review work and out of scope for these contract tests. The S2 contract is: `handleBgCommand` calls `selectBgTerminalBackend` and emits the right messages per REQ-D10/D11 — that wiring is verified by code review + the existing `handleBgStop` / `handleBgOpen` / `handleBgResult` tests that already exercise the dispatch surface. The 4 new tests prove the SELECTOR contract that `handleBgCommand` depends on. Then **EDIT** anchored inside `main()` to add 4 new test calls before the final `console.log("P4-5 bg-commands tests passed");`. `ANCHOR:` `await test("P4-6: reset cleans polling state", testResetCleansState);\n\tconsole.log("P4-5 bg-commands tests passed");` → `REPLACE:` `await test("P4-6: reset cleans polling state", testResetCleansState);\n\tawait test("P5b-1-S2: BgCommandFallsThroughToTmux", testBgCommandFallsThroughToTmux);\n\tawait test("P5b-1-S2: BgCommandReportsNoneAvailable", testBgCommandReportsNoneAvailable);\n\tawait test("P5b-1-S2: BgCommandListsProbedBackendsWhenAllUnavailable", testBgCommandListsProbedBackendsWhenAllUnavailable);\n\tawait test("P5b-1-S2: BgBeforeSessionStart", testBgBeforeSessionStart);\n\tconsole.log("P4-5 bg-commands tests passed");`. | `node agents/test-fixtures/test-bg-commands.mjs` exits 0 and prints the 4 new `✓ P5b-1-S2: ...` lines, then the final `P4-5 bg-commands tests passed` (which the v2.2 wording left unchanged). Negative control: temporarily change `testBgCommandListsProbedBackendsWhenAllUnavailable`'s `assert.equal(result.probed.length, 2, ...)` to `assert.equal(result.probed.length, 99, ...)` and confirm exit non-zero (probes the probed-array assertion is wired); restore. ALSO: temporarily change `testBgCommandFallsThroughToTmux`'s `assert.equal(result.backend.name, "tmux", ...)` to `assert.equal(result.backend.name, "cmux", ...)` and confirm exit non-zero (probes the preference fall-through assertion is wired); restore. |
