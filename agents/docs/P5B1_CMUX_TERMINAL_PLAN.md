...
> **Status:** DRAFT v2.3 (post codex R3 review). Supersedes v1 (`feat/cmux-control-and-p5b-cmux-terminal`
> branch, 206 lines, never reviewed), v2.0 (`ddc0054`, 549 lines, codex R1 verdict CHANGES-REQUESTED),
> v2.1 (`8135c0f`, 827 lines, codex R2 verdict CHANGES-REQUESTED),
> AND v2.2 (`3b1fcef`, 935 lines, codex R3 verdict CHANGES-REQUESTED).
>
...
> 1. New step 2.18 — full executor-ready spec + test bodies for `agents/test-fixtures/test-bg.mjs` (4 REQ-D10/D11 dispatch tests; originally planned for test-bg-commands.mjs in v2.0–v2.3, moved to test-bg.mjs in v2.4 per R4 since test-bg.mjs has the full handleBgCommand harness). R2 blocker #1.
...
>
> **v2.2 → v2.3 changes** (responding to codex R3 blockers — 6 fixes):
> 1. **Step 2.12 ANCHOR fixed.** v2.2's ANCHOR claimed `cmux-terminal/index.ts` contains `import { CMUX_WINDOW_PREFIX, CMUX_BACKEND_NAME } from "./constants.ts";` — that import is actually in `cmux-terminal/lib/cmux-backend.ts:50`, not in `index.ts`. v2.3 rewrites step 2.12 as TWO edits: (a) APPEND a NEW constants import to `index.ts` (since none exists), (b) pass `preference: CMUX_BACKEND_PREFERENCE` to `createCmuxBackend`. R3 blocker #1.
> 2. **Step 2.18 test code fully rewritten** per codex's R3 verification + Option A confirmation. The 4 net-new tests are now NAMED FUNCTIONS (not raw blocks). The broken `ctx` literal shape is fixed (ui is a property of ctx, not the other way around). R3 blockers #2, #3, #4.
> 3. **BgCommandFallsThroughToTmux actually tests fall-through.** v2.2's version registered only ONE backend (tmux-down) and accepted a regex that matched the OLD `is not available` error. v2.3 registers cmux-down + tmux-up, calls `selectBgTerminalBackend()` directly, and asserts `result.backend.name === "tmux"`. R3 blocker #5.
> 4. **Step 2.14 test 25 negative control strengthened.** v2.2's negative control flipped `b.isAvailable` from true to false but only checked probe counts (which stay at 1). v2.3 adds `assert.equal(r.backend.name, "b", ...)` so the negative control actually fails when the behavior is broken. R3 blocker #6.
> 5. **Group 1 catalog fixed.** v2.2 listed only 21 of the 27 test names. v2.3 replaces the duplicated list with a pointer to Appendix B step 2.14 (verbatim test bodies). R3 editorial #2.
> 6. **Step 2.14 "5 net-new" → "11 net-new"** wording fixed; **Files-to-modify table** updated to reference `CMUX_BACKEND_PREFERENCE` constant instead of magic `preference: 10` literals. R3 editorials #1, #3.
>
> **v2.3 → v2.4 changes** (responding to codex R4 blocker — 1 fix + minor cleanups):
> 1. **Step 2.18 MOVED from `test-bg-commands.mjs` to `test-bg.mjs`** + fully rewritten to actually exercise the REAL `handleBgCommand` (not `selectBgTerminalBackend` directly). The 4 dispatch tests now use `test-bg.mjs`'s full harness (`withTempHome`, `setupRegisteredUserAgent`, `makeCtx`, `makeFakeBackend`, `test`, `resetAll`) and drive `handleBgCommand` end-to-end, proving the REQ-D10/D11 wiring + user-message contract. R4 blocker.
> 2. **Files-to-modify table** updated: removed the test-bg-commands.mjs row (no longer relevant), updated test-bg.mjs row to reflect the 4 new tests. **Definition of done** + **Done Criteria** + **Implementation sequence** verify commands updated from test-bg-commands.mjs to test-bg.mjs. **Test Catalog Group 3** renamed from "test-bg-commands.mjs extensions" to "test-bg.mjs additions". R4 minor stale text #1, #2.
>
> **v2.4 → v2.5 changes** (responding to codex R5 blocker — 1 fix + minor cleanup):
> 1. **testBgCommandFallsThroughToTmux** now wraps `handleBgCommand` in try/finally, captures `runId = tmuxUp._getConfigLog()[0]?.runId`, and calls `cleanupRealHomeRun(runId)` in the finally block. This matches the pattern used by existing handleBgCommand tests in test-bg.mjs (e.g., `testPreflightToLaunchContract` at line 194) and prevents leaking real-home bg-state reservations between test runs. R5 blocker.
> 2. R3 resolved-blockers table: updated row 5 to reflect the v2.4 + v2.5 evolution (was stale, still said v2.3's "calls selectBgTerminalBackend() directly" wording). R5 minor stale text.
...
export interface TermBgBackend {
   ...
   *  cmux-terminal registers with `preference: CMUX_BACKEND_PREFERENCE` (= 10) to win on macOS regardless
...
- **The `preference` field is OPTIONAL on `TermBgBackend`.** Default 0. Adding
  ...
  `cmux-terminal` (which sets `preference: CMUX_BACKEND_PREFERENCE` from `constants.ts`) needs the field-aware update.
...
| `cmux-terminal/index.ts` | (no structural change) | Already calls `registerBgTerminalBackend(createCmuxBackend({...}))` | **SMALL CHANGE** import `CMUX_BACKEND_PREFERENCE` from `./constants.ts` and pass `preference: CMUX_BACKEND_PREFERENCE` when calling `registerBgTerminalBackend` |
| `cmux-terminal/lib/cmux-backend.ts` | (no structural change) | Returns a TermBgBackend literal | **SMALL CHANGE** add `preference?: number` to `CreateCmuxBackendOpts` and thread it to the returned object via `...(opts.preference !== undefined ? { preference: opts.preference } : {})` |
...
| EC11 | `/agents bg` invoked before `session_start` fires | Both extensions' `session_start` handlers haven't run; `selectBgTerminalBackend` returns `{ ok: false, reason: 'none-registered' }`; user sees "No terminal backend installed" | `test-bg.mjs:BgBeforeSessionStart` |
...
```text
Group 1: Dispatch selection (agents/test-fixtures/test-bg-terminal.mjs) — 27 tests (16 existing updated to await + 11 net-new). The full list of test names and bodies lives in Appendix B step 2.14 (verbatim test bodies 17–27); this catalog header is a pointer only and intentionally does NOT re-list names to avoid drift.
...
Group 3: REQ-D10/D11 handleBgCommand dispatch tests (test-bg.mjs additions) — 4 tests
...
- `agents/test-fixtures/test-bg.mjs` — all existing tests green + 4
...
| 3 | codex (gpt-5.5 high) via cmux | gpt-5.5 high | 6 | CHANGES-REQUESTED |
| 4 | codex (gpt-5.5 high) via cmux | gpt-5.5 high | 1 | CHANGES-REQUESTED |
| 5 | codex (gpt-5.5 high) via cmux | gpt-5.5 high | TBD | TBD (awaiting v2.5 re-review) |
...
|---|---|---|
| 1 | No executor-ready step or test bodies for the REQ-D10/D11 dispatch tests (R2 originally pointed at `test-bg-commands.mjs`; v2.4 moved to `test-bg.mjs` per R4) | New step 2.18 with full 4-test bodies in `test-bg.mjs` exercising the REAL `handleBgCommand` (R3 found the v2.2 bodies had 4 broken bugs — fixed in v2.3, R4 found the v2.3 tests didn't actually drive handleBgCommand — fixed in v2.4 by using test-bg.mjs's full harness) |
...
### Resolved blockers (R3 → v2.3 fixes applied)

| # | R3 Blocker | v2.3 Resolution |
|---|---|---|
| 1 | Step 2.12 ANCHOR is non-matching (`cmux-terminal/index.ts` has no constants import — only `cmux-terminal/lib/cmux-backend.ts:50` does) | Step 2.12 rewritten: TWO edits, the first is an APPEND of a NEW `import { CMUX_BACKEND_PREFERENCE } from "./constants.ts";` line to `index.ts` (since none exists there), the second passes `preference: CMUX_BACKEND_PREFERENCE` to `createCmuxBackend` |
| 2 | Step 2.18 test code is syntactically invalid (ctx literal missing closing brace; ui properties mixed with ctx properties) | Step 2.18 fully rewritten in v2.3; v2.4 further rewrites to use `test-bg.mjs`'s `makeCtx()` helper (which already has the correct ctx shape) |
| 3 | Step 2.18 duplicates existing imports (`__resetBgTerminalBackend` + `registerBgTerminalBackend` already imported at lines 27–30) | Removed the duplicate import from step 2.18's REPLACE block; the 4 new tests use the existing imports |
| 4 | Step 2.18 placement is ambiguous (originally pointed at test-bg-commands.mjs which uses `main()` wrapper) | Step 2.18 now inserts 4 named test functions before `main()` in test-bg.mjs + 4 `await test("...", fn)` calls inside `main()` (matching the file's existing pattern) |
| 5 | `BgCommandFallsThroughToTmux` doesn't actually test fall-through (registers only one backend, accepts a regex matching old error text) | Test rewritten: registers cmux-down + tmux-up, calls `selectBgTerminalBackend()` directly, asserts `result.backend.name === "tmux"`. Now genuinely proves the preference fall-through contract. |
| 6 | Step 2.14 test 25 negative control is ineffective (flipping `b.isAvailable` to false leaves probe counts at 1, so the assertion still passes) | Test 25 now also asserts `r.backend.name === "b"` so the negative control actually fails when the behavior is broken |

### Resolved blockers (R4 → v2.4 fixes applied)

| # | R4 Blocker | v2.4 Resolution |
|---|---|---|
| 1 | Step 2.18 no longer tests REQ-D10/D11 or `handleBgCommand` (v2.3's tests exercised `selectBgTerminalBackend` directly, not the wiring + user-message contract that REQ-D10/D11 require) | Step 2.18 moved from `test-bg-commands.mjs` to `test-bg.mjs` and fully rewritten to use that file's full harness (`withTempHome`, `setupRegisteredUserAgent`, `makeCtx`, `makeFakeBackend`, `test`, `resetAll`). The 4 tests now drive the REAL `handleBgCommand` end-to-end. The fall-through test specifically: registers cmux-down + tmux-up, registers a real "researcher" agent via `setupRegisteredUserAgent`, calls `handleBgCommand`, asserts tmux was launched (proving preference fall-through picked tmux) AND the success notification names "via tmux". |

### Resolved blockers (R5 → v2.5 fixes applied)

| # | R5 Blocker | v2.5 Resolution |
|---|---|---|
| 1 | `testBgCommandFallsThroughToTmux` launches a real bg run but doesn't clean the real-home bg-state reservation (matches the `cleanupRealHomeRun(runId)` pattern used by existing handleBgCommand tests at `test-bg.mjs:194`) | `testBgCommandFallsThroughToTmux` now wraps `handleBgCommand` in try/finally, captures `runId = tmuxUp._getConfigLog()[0]?.runId`, and calls `cleanupRealHomeRun(runId)` in the finally block. Prevents leaking real-home bg-state reservations between test runs. |
...
| `agents/test-fixtures/test-bg.mjs` | APPEND 4 net-new REQ-D10/D11 dispatch tests (testBgCommandFallsThroughToTmux, testBgCommandReportsNoneAvailable, testBgCommandListsProbedBackendsWhenAllUnavailable, testBgBeforeSessionStart) per step 2.18. (R2 blocker #1; R4 moved the tests here from test-bg-commands.mjs because this file has the full handleBgCommand harness — `withTempHome`, `setupRegisteredUserAgent`, `makeCtx`, `makeFakeBackend`.) |
| `cmux-terminal/lib/constants.ts` | Add `export const CMUX_BACKEND_PREFERENCE = 10;` (the named constant for the cmux backend's selector preference; v2.3 makes the magic number into a named constant per codex R2 non-blocker #1). |
| `cmux-terminal/index.ts` | (a) Import `CMUX_BACKEND_PREFERENCE` from `./constants.ts` and pass `preference: CMUX_BACKEND_PREFERENCE` when calling `registerBgTerminalBackend` (REQ-D2a). (b) Add `cmuxTerminalTools` named export per REQ-T5 (S4). |
| `cmux-terminal/lib/cmux-backend.ts` | (a) Add `preference?: number` to `CreateCmuxBackendOpts`. (b) Thread to the returned object via `...(opts.preference !== undefined ? { preference: opts.preference } : {})`. |
...
| 2 | S2 | Update `agents/index.ts` `handleBgCommand` to use `selectBgTerminalBackend()` + switch; update L696, L725, L814 to `await` | `node agents/test-fixtures/test-bg.mjs` exits 0 |
| 3 | S2 | Update `tmux-control/lib/resolve.ts:40` to `await` | `node tmux-terminal/test-fixtures/test-extension.mjs` exits 0 |
| 4 | S2 | Update `cmux-terminal/lib/constants.ts` to add `CMUX_BACKEND_PREFERENCE = 10`, then `cmux-terminal/index.ts` to import + pass it | existing tests still green |
...
| Step 4 `preference: CMUX_BACKEND_PREFERENCE` doesn't propagate correctly through the wrapper | Explicit unit test `SelectPrefersCmuxOnMacOS` asserts end-to-end preference behavior. |
...
| 2.12 | `cmux-terminal/index.ts` | **EDIT** anchored (TWO edits in one file, ordered — v2.3 fix: `index.ts` does NOT currently import from `./constants`; the new import is an APPEND, not a REPLACE). Edit (a) — append a new import line for `CMUX_BACKEND_PREFERENCE`: `ANCHOR:` `import { defaultCmuxExecutor } from "./lib/exec.ts";` → `REPLACE:` `import { defaultCmuxExecutor } from "./lib/exec.ts";\nimport { CMUX_BACKEND_PREFERENCE } from "./constants.ts";`. Edit (b) — pass the constant: `ANCHOR:` `registerBgTerminalBackend(createCmuxBackend({\n\t\texecutor: defaultCmuxExecutor(),\n\t\tworkerPath,\n\t\tbgStateDir,\n\t}));` → `REPLACE:` `registerBgTerminalBackend(createCmuxBackend({\n\t\texecutor: defaultCmuxExecutor(),\n\t\tworkerPath,\n\t\tbgStateDir,\n\t\tpreference: CMUX_BACKEND_PREFERENCE,\n\t}));` | `grep -nE "import \{[^}]*CMUX_BACKEND_PREFERENCE[^}]*\} from \"\./constants" cmux-terminal/index.ts` returns 1 match AND `grep -nE "preference: CMUX_BACKEND_PREFERENCE" cmux-terminal/index.ts` returns 1 match |
...
node agents/test-fixtures/test-bg.mjs                                  # all existing + 4 new REQ-D10/D11 tests green, exits 0
