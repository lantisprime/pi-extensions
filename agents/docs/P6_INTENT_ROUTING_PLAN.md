...
**Template-conformance pass (post-merge of the falsifiable-verify / non-deterministic-guard
template hardening), applied across P6-2/3a/3b/4:** every `node … exits 0` Verify cell now names
its observed→expected value **and the in-suite negative control** that proves falsifiability
(Positive-obligation rule), rather than trusting a bare exit code; 2.1's grep is annotated as a
landing marker (real delegation ⇒ E2E-6); the real-classifier E2E scenarios (E2E-3/E2E-4 and the
enabled arm of E2E-6) became **3b** rate thresholds (`≥4 of 5`, report the rate, no retry-to-green)
with a calibration note. Also fixed a **pre-existing ordering defect**: 3b.6 and 4.2–4.5 verified by
`node`-ing a test file not CREATEd until 3b.7 / 4.6 — their Verify is now a landing-grep with the
behavioral run deferred to the create step (mirrors the P6-0b 0b.10–0b.14 pattern). No test count
changed (Groups 3/5/6/7 unchanged).
...
| E2E-3 | `/agents do` routes review intent | `/agents do "review the parser for bugs"` (high-confidence) | **3b — classification IS the property under test, so NO retry-to-green.** Run the classify **N=5×** and assert routed agent = `reviewer` in **≥4 of 5** runs (rate threshold); **report the observed rate**, do not stop at the first green. Heuristic fallback to reviewer counts as a pass. A real review returned; routing metadata shows engine + reason; auto-ran (read-only ≥0.8). If the rate falls below 4/5 on another model/provider, tag the scenario `UNGUARDED-IN-CI` and name the manual check that covers it. |
| E2E-4 | `/agents do` routes recon intent | `/agents do "where is config loaded?"` | **3b rate (no retry-to-green):** run **N=5×**, assert routed agent = `scout` in **≥4 of 5** runs and **report the observed rate**; real findings returned; sub-threshold on another model/provider ⇒ tag `UNGUARDED-IN-CI` + name the manual check |
...
| E2E-6 | LLM classifier real call vs fallback | run with classifier enabled (real `pi` classify) **N=5×**, then with classifier forced to error | **enabled (3b — JSON-contract compliance is non-deterministic: the LLM may emit prose that `parseClassifierOutput` rejects → fallback):** `engine:"llm"` with a valid candidate in **≥4 of 5** runs (report the rate; no retry-to-green; sub-threshold ⇒ tag `UNGUARDED-IN-CI` + manual check). **forced-error (deterministic):** `engine:"heuristic-fallback"`, still runs |
...
**3b threshold calibration (E2E-3/E2E-4/E2E-6):** the `≥4 of 5` gates assume **near-unambiguous
prompts** (true per-run rate ≥0.9, where `P(≥4/5) ≈ 0.98`). They are deliberately worded for
prompts a competent classifier should almost always get right; the mandatory **report-the-rate**
clause is what surfaces drift toward the self-flaking zone (at a true 0.8 rate, `≥4/5` already
self-flakes ~26% — the same statistical trap the template's consecutive-greens calibration note
warns about). If a real run reports a rate near the threshold, raise N or tag `UNGUARDED-IN-CI`
rather than re-rolling to green.
...
| 2.5 | `agents/test-fixtures/test-intent-classifier.mjs` | **CREATE.** Group 3 — **all 11 tests** — with an injected `runClassifier` stub that captures `(invocation, limits)`: `testClassifierArgs_emitsNoTools`, `testClassifierArgs_omitsToolsFlag`, `testClassifierArgs_noSession` (M2 — was missing; assert argv includes `--no-session`), `testClassifierArgs_thinkingOff`, `testClassifierArgs_boundedLimits` (assert the captured `limits.timeoutMs===20000`, `limits.stdoutLimit===65536`, `limits.maxResultChars===512`), `testClassifierArgs_overrideModelOnly` (`overrideModel:"x"` → argv has `--model x`, no extra `--thinking`), `testClassifierArgs_overrideThinkingIgnoredWithWarning` (`overrideThinking:"high"` → `warnings.length===1`, argv `--thinking off` unchanged), `testResolve_llmPrimary`, `testResolve_fallbackOnSpawnError`, `testResolve_fallbackOnBadJson`, `testResolve_fallbackOnUnknownAgent`. | `node agents/test-fixtures/test-intent-classifier.mjs` exits 0 — all 11 pass; observed captured `invocation.argv` includes `--no-tools` / excludes `--tools` and `limits` equals `CLASSIFIER_LIMITS` (`timeoutMs 20000`, `stdoutLimit 65536`, `maxResultChars 512`); falsifiability carried **in-suite** by `testClassifierArgs_omitsToolsFlag` (a builder emitting `--tools` ⇒ red) and `testResolve_fallbackOnBadJson` (a parser accepting garbage ⇒ red), so a stubbed unit exits non-zero |
| 2.6 | `agents/test-fixtures/run-p6-2-tests.sh` | **CREATE.** runs `test-intent-classifier.mjs`. | `bash agents/test-fixtures/run-p6-2-tests.sh` exits 0 — runs Group 3 incl. its in-suite negative controls (`testClassifierArgs_omitsToolsFlag`, `testResolve_fallbackOn*`); non-zero if any of the 11 fail |
...
| 3a.2 | `agents/lib/run-resolver.ts` | **EDIT.** `ANCHOR:` verbatim from `	const record = resolved.record;` (line 119) through `	await executeChildRun(currentParsed.spec, parsed.task, ctx, record.source, parsed.profileOverride);` (line 141) → `REPLACE:` `	await runResolvedTarget(resolved.record, parsed.task, ctx, diagnostics, parsed.profileOverride);` (line 119 is **deleted** — its `record` becomes the function parameter; the whole 119–141 tail collapses to one call). | `node agents/test-fixtures/test-p3f-4.mjs` exits 0 — the pre-existing run-path assertions on the child spec/task/profile stay green; a wrong `record.name`/`task`/`profileOverride` substitution changes the observed child invocation and turns them red (this is the zero-behavior-change proof) |
| 3a.3 | `agents/test-fixtures/run-p6-3-tests.sh` | **CREATE.** `#!/usr/bin/env bash`, `set -euo pipefail`; runs the **run-path** regressions that actually exercise `runAgentCommand`/`executeChildRun` — `node "$(dirname "$0")/test-p3f-4.mjs"` and `node "$(dirname "$0")/test-subagent-tool.mjs"` (NOT `test-registry-gate.mjs`, which only unit-tests `canRunAgent` and never reaches the extracted code — M1). `chmod +x`. | `bash agents/test-fixtures/run-p6-3-tests.sh` exits 0 — both run-path regressions (`test-p3f-4`, `test-subagent-tool`) green; non-zero if the extraction altered any observed child invocation |
...
| 3b.6 | `agents/index.ts` | **EDIT.** `ANCHOR:` `			if (parsed.action === "run") {` → `REPLACE:` add directly above it: `if (parsed.action === "do") { await runIntentCommand(parsed.rest, ctx, diagnostics); return; }` + the original line. (Routing keys off `parsed.action`; 3b.5 only adds completion — but keep both.) | `grep -n 'parsed.action === "do"' agents/index.ts` (dispatch line landed; the behavioral `node …/test-intent-command.mjs` run is deferred to **3b.7**, which CREATEs that suite — do not `node` it here, the file does not exist yet) |
| 3b.7 | `agents/test-fixtures/test-intent-command.mjs` | **CREATE.** Groups 5+6 with injected `ctx`/`diagnostics`/classifier (the catalog tests, incl. `testDo_nonTuiNeverSpawnsClassifier` asserting `ctx.hasUI=false` → classifier `callCount===0`, `testDo_emptyTaskUsage`, `testDo_registeredAutoRunHighConfidence`, `testDo_autoRunRequiresReadOnlyTools`, `testDo_profileFlagParsing`). | `node agents/test-fixtures/test-intent-command.mjs` exits 0 — Groups 5+6 green; observed = the captured classifier `callCount` and routed agent under injected `ctx`/`diagnostics`; falsifiable **in-suite** by `testDo_nonTuiNeverSpawnsClassifier` (a missing fail-closed ⇒ `callCount===1` ⇒ red) and `testDo_gateDeniesUnregistered` (a bypassed `canRunAgent` ⇒ red) |
| 3b.8 | `agents/test-fixtures/run-p6-3-tests.sh` | **EDIT.** `ANCHOR:` the `test-subagent-tool.mjs` line (added in 3a.3) → `REPLACE:` same + a line running `test-intent-command.mjs`. | `bash agents/test-fixtures/run-p6-3-tests.sh` exits 0 — run-path regressions + the new Group 5/6 suite (incl. its in-suite negative controls); non-zero if any fail |
...
| 4.2 | `agents/lib/run-resolver.ts` | **EDIT (B3 — re-anchored to the correct `else` branch + return type, 3 sub-edits in `parseRunArgs`).** (i) `ANCHOR:` `export function parseRunArgs(input: string): { ok: true; name: string; task: string; profileOverride?: string } | { ok: false; message: string } {` → `REPLACE:` add `; warning?: string` inside the `ok: true` object (after `profileOverride?: string`). (ii) `ANCHOR:` verbatim the `else` branch `	} else {` + `		rest = tokens.slice(1).join(" ");` + `	}` → `REPLACE:` same, plus a following line `	const warning = (tokens[1] !== "--profile" && tokens.slice(1).some((t) => t === "--profile")) ? "--profile must come right after the agent name; treated as task text" : undefined;` (REQ-14: a stray `--profile` when there is **no** leading one — the line-171 guard handles the *repeated*-with-leading case and is left unchanged). (iii) `ANCHOR:` `	return { ok: true, name, task, profileOverride };` → `REPLACE:` `	return { ok: true, name, task, profileOverride, warning };`. **Caller note:** `runAgentCommand` should `if (parsed.warning) ctx.ui.notify(parsed.warning, "warning");` — but that caller edit is its own line; keep it in 4.2 as a 4th sub-edit anchored on `	const parsed = parseRunArgs(input);` → add the notify directly after the `if (!parsed.ok)` block. | `grep -n 'must come right after the agent name' agents/lib/run-resolver.ts` (REQ-14 warning string landed; the behavioral `node …/test-disambiguation.mjs` run is deferred to **4.6**, which CREATEs the suite) |
| 4.3 | `agents/index.ts` | **EDIT (B2 — `formatProfileList` is in index.ts, NOT diagnostics.ts).** `ANCHOR:` verbatim `		if (profile.thinking) parts.push(\`thinking=${profile.thinking}\`);` → `REPLACE:` same line + below it `		if (!profile.model && !profile.thinking) parts.push("effect: none (Pi default)");` (REQ-13 no-op label; `= profileEffect(profile) === "none"`). | `grep -n 'effect: none (Pi default)' agents/index.ts` (REQ-13 label landed; behavioral `node …` run deferred to **4.6**) |
| 4.4 | `agents/lib/diagnostics.ts` | **EDIT (REQ-13 inspect label).** `ANCHOR:` verbatim `		if (record.spec?.profile) lines.push(\`profile: ${record.spec.profile}${record.spec.profile && !BUILT_IN_PROFILES[record.spec.profile] ? " (unresolved in built-in profiles)" : ""}\`);` → `REPLACE:` same line + below it `		if (record.spec?.profile && BUILT_IN_PROFILES[record.spec.profile] && !BUILT_IN_PROFILES[record.spec.profile].model && !BUILT_IN_PROFILES[record.spec.profile].thinking) lines.push("effect: none (Pi default)");`. | `grep -n 'effect: none (Pi default)' agents/lib/diagnostics.ts` (REQ-13 inspect label landed; behavioral `node …` run deferred to **4.6**) |
| 4.5 | `agents/lib/diagnostics.ts` | **EDIT (REQ-12 doctor collision warning).** `ANCHOR:` verbatim `	for (const entry of diagnostics.registryOnlyEntries) issues.push(\`${entry.name} [${entry.source}] registry entry has no matching discovered file: ${entry.canonicalPath}\`);` → `REPLACE:` same line + ABOVE it `	for (const collidingName of agentProfileNameCollisions(diagnostics, Object.keys(BUILT_IN_PROFILES))) issues.push(\`Agent '${collidingName}' shares a name with a built-in profile; '/agents run ${collidingName}' uses the agent, not the profile.\`);`. | `grep -n 'shares a name with a built-in profile' agents/lib/diagnostics.ts` (REQ-12 doctor warning landed; behavioral `node …` run deferred to **4.6**) |
| 4.6 | `agents/test-fixtures/test-disambiguation.mjs` | **CREATE.** Group 7: `testProfileEffect_classifies` (imports `profileEffect` from `../lib/intent-router.ts`), `testDoctor_warnsAgentProfileCollision`, `testProfiles_labelsNoOpProfile`, `testInspect_showsNoOpProfile`, `testParseRun_warnsMisplacedProfile`. | `node agents/test-fixtures/test-disambiguation.mjs` exits 0 — Group 7 green; observed = the rendered profile labels / doctor `issues` / `parseRunArgs().warning` captured from the real functions; falsifiable **in-suite** by `testParseRun_warnsMisplacedProfile` (no warning emitted ⇒ red) and `testDoctor_warnsAgentProfileCollision` (missing collision issue ⇒ red). This is the **first** behavioral run of the suite — 4.2–4.5 land their edits and grep-verify; the contract is proven here |
| 4.7 | `agents/test-fixtures/run-p6-4-tests.sh` | **CREATE.** `#!/usr/bin/env bash`, `set -euo pipefail`, `node "$(dirname "$0")/test-disambiguation.mjs"`. `chmod +x`. | `bash agents/test-fixtures/run-p6-4-tests.sh` exits 0 — runs Group 7 incl. its in-suite negative controls; non-zero if any fail |
