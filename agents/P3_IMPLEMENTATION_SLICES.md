...
| `P5c-2-S5` | `mode: "literal"\|"keys"` for `tmux_send` (surface seam already in `send.ts` since S1; keys mode defaults `pressEnter:false`) | `lib/send.ts`, `index.ts` | **OPEN (top priority, next)** | — | serial (after S3/S4 finalize send.ts) |
