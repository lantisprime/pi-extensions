...
### P5F disk-backed per-project trust reader (PLAN ACCEPTED — PR #143, commit `0ca3ee6`; P5F-1 NEXT)
- **Status: planning-only merge (no implementation yet).** Plan: `agents/docs/P5F_DISK_BACKED_TRUST_READER_PLAN.md`. Pass-1 review artifact: `agents/docs/P5F_REVIEW.md`. Milestone episode: `20260704-141556-p5f-7968`.
...
- **3 slices**: P5F-1 (pure reader extraction, zero callers), P5F-2 (writer + resolver), P5F-3 (read-side wiring). 23 unit tests + 2 static + 1 UNGUARDED-IN-CI smoke + 1 UNGUARDED-IN-CI manual grep.
...
- **Slice ladder**:
  - `P5F-1` — pure reader extraction (zero production callers). Files: `agents/lib/bg-trust.ts` (new), `agents/test/test-bg-trust.mjs` (new), `agents/test/run-bg-trust-tests.sh` (new). 12 unit tests (Group 1 = 10, Group 2 = 2). Appendix B step 1.1 has full verbatim `bg-trust.ts` source (low-capability-executor-ready); step 1.2 has 4 verbatim tests + 8-test contract table (high-capability executor scope). Hard stop: no production caller; existing suite green. Branch: `feat/p5f-1-trust-reader`.
  - `P5F-2` — writer + resolver. APPEND `writeProjectTrustStore` + `resolveDefaultBackend` to `bg-trust.ts`; 7 tests (Group 3 = 4 incl. atomic-failure + EC5 rotation, Group 4 = 3). Step-tables deferred to post-P5F-1-review.
  - `P5F-3` — `/agents bg` read-side wiring + preflight snapshot source. EDIT `index.ts` L713 region (one `resolveDefaultBackend` call in the flag-absent branch — focused-review slice) + EDIT `bg-preflight.ts` L86-102 (source the existing `projectTrusted: boolean` from disk; no shape change). 4 unit tests (Group 5) + 1 UNGUARDED-IN-CI smoke. Step-tables deferred to post-P5F-1-review.
