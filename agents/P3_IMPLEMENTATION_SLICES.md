...
### P5c-2 tmux-control TUI automation surface (S1 + S2 SHIPPED; S3–S6 OPEN)
...
- Stats: 19 REQ rows, 29 unit tests + 3 smoke steps (14 unit + 2 smoke landed in S1; **18** `waitForWindow` unit tests + 2 real-tmux smoke landed in S2 — 9 initial + 9 added after Claude + Codex review). 12 real-tmux smoke steps total in `test-real-tmux-smoke.mjs` (4 new in this PR) + 2 Path A bracketed-paste marker checks (separate `test-pathA.mjs` script). Existing REQ-13 import-guard gets an additional argv-only static guard (REQ-17) with red-then-green self-check.
- **S2 review cycle (Claude + Codex via tmux, 3 rounds)**:
  ...
  - **Single-commit amendment**: S2 originally committed as `214f888`, amended to `5f2ffeb` post-review per project pattern (cf. S1 commit `68c27d7` which absorbed research-driven fixes). Original hash orphaned. On `feat/p5c-2-foundations` (PR #109), the cherry-pick equivalent hash is `d8ee10b`.
- S2 file conflict analysis (post-S2 commit `214f888`):
  ...
  - **S5** is a new file `lib/keyscheck.ts` — no overlap with S1 or S2.
